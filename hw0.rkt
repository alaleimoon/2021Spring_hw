...
; ********************************************************
; CS 201 HW #0  DUE Wednesday 2/17/2021, 11:59 pm
...
(define (pds mass) (inexact->exact (floor (* mass 2.2))))
(define (ons mass) (inexact->exact (floor (* 1.6 (remainder (floor (* 2.2 (* 10 mass))) 10)))))

(define (kg2lb mass)
  (list
   (pds mass)
   (if (equal? 1 (pds mass)) 'pound 'pounds)
   (ons mass)
   (if (equal? 1 (ons mass)) 'ounce 'ounces)
   ))
...
; issues: October, November, December 2020 or January, February 2021 See http://cacm.acm.org/
...
;; 2/3/2021 SS I redefined test to provide less confusing output.
