...
; ********************************************************
; CS 201 HW #3  DUE Thursday, 3/11/2021 at 11:59 pm
...
(define tm-reverse
  (list
   ...
   (ins 'q21 0 'q24 'b 'R)
   (ins 'q24 'b 'q23 0 'L)
   ;move to the empty space for insert
   (ins 'q23 'b 'q26 'b 'L)
   (ins 'q26 1 'q22 'b 'R)
   (ins 'q26 0 'q24 'b 'R)
   (ins 'q26 'x 'q2 'x 'L)
...
(define (i-match? state symbol inst)
  (if
   (and (equal? state (ins-c-state inst)) (equal? symbol (ins-c-symbol inst)))
   #t #f))
...
(define (normalize config)
  (conf
   (conf-state config)
   (nor-helper (conf-ltape config))
   (conf-symbol config)
   (reverse (nor-helper (reverse (conf-rtape config))))
   ))

(define (nor-helper list)
  (if (empty? list)
      '()
      (if (equal? 'b (car list))
          (nor-helper (cdr list))
...
(define (shift-head-left config)
  (conf
   ...
   (if (empty? (conf-ltape config))
      '()
      (reverse (cdr (reverse (conf-ltape config)))))
   (if (empty? (conf-ltape config))
       'b
       (car (reverse (conf-ltape config))))
   (if (and (empty? (conf-rtape config)) (equal? 'b (conf-symbol config)))
       '()
       (cons (conf-symbol config) (conf-rtape config)))
   ))

(define (shift-head-right config)
  (rev-config (shift-head-left (rev-config config))))

;reverse the left tape with right tape
(define (rev-config config)
  (conf
   (conf-state config)
   (reverse (conf-rtape config))
   (conf-symbol config)
   (reverse (conf-ltape config))))
...
(define (next-config mach config)
  (define current-ins (i-lookup (conf-state config) (conf-symbol config) mach))
  (if (halted? mach config)
      config
      ((if (equal? (ins-dir current-ins) 'L)
           shift-head-left
           shift-head-right)
       (normalize
        (change-state (ins-n-state current-ins)
                      (write-symbol (ins-n-symbol current-ins)
                                    config))))
  ))
...
(define tm-sort
  (list
   ;add a start boundary to the list
   (ins 'q1 0 'q1 0 'L)
   (ins 'q1 1 'q1 1 'L)
   (ins 'q1 'b 'q11 'y 'R)
